...
export const InsightProductCardSkeleton: FC<{
  type?: "search" | "details";
}> = ({ type = "search" }) => {
  return (
    <div className="w-full">
      <div className="w-full border-[#1A1A1D] dark:border-inherit border rounded-[2rem] px-4 py-2">
        <div
          className={cn(
            "pt-4 md:pt-6 flex flex-col-reverse justify-between h-full",
            type === "search" ? "md:flex-row" : "md:flex-row-reverse"
          )}
        >
          ...
          <div
            className={cn(
              "h-full w-full md:max-w-sm flex justify-center",
              type === "search" ? "md:justify-end" : "md:justify-start"
            )}
          >
            <div className="flex flex-col pb-6 px-2 md:px-4">
